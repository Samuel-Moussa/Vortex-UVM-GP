...
|---|---|---|---|
| **G1** | **Cache behaviour: hit / miss / eviction / writeback / MSHR occupancy / flush — at every level (L1 I&D, L2, L3)** | No `cp_*` matching `hit\|miss\|evict\|writeback\|mshr\|flush` exists in the collector or any probe. The only cache-adjacent coverpoints are `cp_id_route` / `cross_type_route`, which sample **routing tag bits**, not cache events. | **The most significant gap.** The cache hierarchy is a major part of the design and one of the likeliest places for real bugs (arbitration, MSHR reuse, eviction races). Today its verification rests entirely on *code* coverage plus end-state/lockstep equivalence — i.e. "a correct program produced a correct answer", which a cache can achieve while still having latent corner-case bugs. |
