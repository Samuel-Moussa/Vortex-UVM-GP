...
\begin{table}[t]
	\caption{Lockstep configuration matrix (all lane-exact: 0 mismatches,
		...
		per-run raw logs were not retained. The 8W/4T point predates the
		reset-relay and parameter-audit fixes and documents the comparator
		mechanics rather than current-design status.}
	\label{tab:matrix}
	\centering
	\small
	\begin{tabular}{lr}
...
\begin{table}[t]
	...
	\begin{threeparttable}
		...
		\begin{tablenotes}\footnotesize
			...
			relay-fixed L2/L3 bank (cache-focused suite, 2026-09-11)
			later replaced its passthrough waivers with genuinely
			exercised shared-hierarchy coverage (85.1\% total on the
			enlarged denominator).
		\end{tablenotes}
	\end{threeparttable}
\end{table}
