...
consistent with the different pins and oracles involved. Mutation-based checker qualification is established
...
no third-party code is used. The divergence axis emits structured
branch trees with controlled split depths; the memory axis emits
stride/scatter/bank-hostile patterns. Two further axes (barrier
topology, VOTE/SHFL) are \emph{declared but not implemented} and
reported as open. A ten-seed sweep across nine riscv-dv profiles (90
...
\begin{table}[t]
	\caption{Coverage banks (QuestaSim; per-configuration, never
		...
		fixed design; cross-configuration branch comparisons are invalid
		and not made.}
	\label{tab:coverage}
	\centering
	\small
...
70{,}498+ cycles), and two coalescing classes. Deliberate simtgen
programs closed all three (Fig.~\ref{fig:simtclosure}): depth 4/4, real
bank conflicts surfaced, and coalescing 3/3. Two notes travel with this result: (a)~the closures
were verified in \emph{isolated merges}; a full-suite re-run folding
them into the frozen banks is documented as pending, so the totals of
Table~\ref{tab:coverage} do not yet include them; (b)~the
bank-conflict ``gap'' was in fact a \emph{coverage-model defect}: the
...
reachable and honest (169 conflict hits across six programs). A
...
by experiment. The frozen bank's seven unhit covergroup bins decompose
as six SIMT-layer bins---the divergence, bank-conflict, and coalescing
targets above, closed only in the isolated merges---plus structural
unreachable-by-stimulus classes left red by rule rather than waived;
full fold-in awaits a re-run campaign. The marginal picture per
generator kind on the frozen model: ninety additional riscv-dv
programs closed zero bins (the seed-farm result), while six deliberate
simtgen programs closed all three open SIMT targets---stimulus
