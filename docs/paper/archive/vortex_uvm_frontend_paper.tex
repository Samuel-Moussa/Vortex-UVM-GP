...
Neither is supportable. The randomized generator is seed-controlled, and we
report a ten-seed sweep below---but that sweep produced no additional
coverage, which sharpens rather than softens the limitation. The bus responder always
...
differ, so a merged database is not meaningful. The primary-configuration
bank is measured on the reset-corrected design; the two-cluster and L2/L3
banks predate that correction and are therefore not comparable to it on
...
That configuration was banked separately (51 runs, all passing, 93.2\% total).
It is lower than the two-cluster bank without the shared levels, and the reason
is instructive rather than disappointing: enabling two cache levels adds a large
amount of new control logic---arbiters, flush state machines, miss-status
handling---while exactly one kernel of fifty-one targets it. Condition coverage
there is 79.3\%. The honest summary is that the shared hierarchy was
\emph{reached and validated, not thoroughly exercised}: both halves of that
sentence matter.
...
\subsection{Seed volume: a measured negative result}
\label{sec:seedfarm}

The committed regression pins one seed per profile, which is reproducible but
is directed testing in a random costume. To measure what that costs, we swept
ten seeds across all nine generator profiles---ninety additional runs.

Every generated program was confirmed distinct by content hash before the
result was interpreted; a seed sweep in which the seed silently fails to take
effect produces ninety passing runs and is indistinguishable from a working
one. Program lengths ranged from 4\,432 to 857\,190 cycles, a factor of 190,
and a factor of four within a single profile.

\textbf{All ninety runs passed}, which is a genuine robustness result: the
design is demonstrably not tuned to the committed seed. \textbf{Coverage did
not move.} Against the fifty-one-run baseline, assertion, branch, condition,
statement and covergroup coverage were bit-identical, and toggle rose by
0.06\%. Total coverage was unchanged at 94.7\%.

We report this because the obvious remedy for weak randomization---run more
seeds---was measured here and returned nothing. The binding constraint is the
generator's \emph{reach}, not its sample count: it emits user-mode integer code
with machine-mode control-register writes removed, so every seed explores the
same region of the state space, and coverage saturates inside that region well
before the tenth seed. No number of seeds reaches the exception paths or error
responses that remain uncovered; those require different \emph{kinds} of
stimulus, not more samples of one kind. Accordingly we separate stimulus
\emph{volume} from stimulus \emph{diversity} in Table~\ref{tab:maturity}, and
identify diversity as the open axis.
...
Random stimulus volume & Moderate (10 seeds/profile) \\
Random stimulus \emph{diversity} & \textbf{Weak} (one generator, one ISA subset) \\
