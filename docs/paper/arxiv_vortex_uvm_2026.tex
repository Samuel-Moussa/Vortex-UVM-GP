...
different pins and oracles involved. We have since exercised their
reported Vortex findings at our pin: X1 reproduces (the PoC fires the
RTL's own invalid-writeback-register assertion), and X2---whose
upstream fix landed entirely in the reference model's
decoder---reproduces as a genuine DUT-vs-SimX divergence in which the
hardware is right and the golden model is wrong; both corroborate the
two flows' complementarity and, for X2, this paper's thesis that the
golden model is itself a verification target. Concurrent with both,
the Ventus
...
\begin{table}[t]
	\caption{Lockstep configuration matrix (all lane-exact: 0 mismatches,
		...
		documents; per-run raw logs were not retained. The 8W/4T point
		predates the reset-relay and parameter-audit fixes and documents
		the comparator mechanics rather than current-design status.}
	\label{tab:matrix}
	\centering
	\small
	\begin{tabular}{lr}
...
rejected for engineering reasons, is documented in the artifact).
\code{simtgen} implements four axes: divergence (structured branch
trees with controlled split depths and asymmetric masks), memory
(stride/scatter/bank-hostile access patterns), barrier topology
(single-warp barriers, deadlock-safe by construction; \code{wspawn} is
deliberately never emitted---it is a runtime-only bootstrap primitive
with its own structural waiver), and VOTE/SHFL (all eight intrinsics
per program). The barrier and VOTE/SHFL axes closed no new
coverage---their targets (\code{cp_vote_shfl_op} 8/8,
\code{cross_sfu_threads}) were already fully covered by dedicated
directed kernels (\code{vote_shfl}, \code{bar_masks},
\code{sfu_masks}); their contribution is generator completeness and
seed-diversity robustness evidence, the same category of result as the
seed farm below. simtgen's measured effect is quantified in
...
\begin{table}[t]
	...
	\begin{threeparttable}
		...
		\begin{tablenotes}\footnotesize
			...
			(51 runs, all passing), also on the pre-fix design. A
			relay-fixed L2/L3 bank (seven-kernel cache-focused suite,
			2026-09-11) replaces the prior passthrough waivers with
			genuinely exercised shared-hierarchy coverage: 85.1\% total
			on the correspondingly enlarged denominator, with live
			cache-probe hit counts verified recursively.
		\end{tablenotes}
	\end{threeparttable}
\end{table}
...
coverage targets were \emph{believed} open: the depth-4 IPDOM
divergence bin
...
(0/3, never sampled in 70{,}498+ cycles), and the coalescing classes of
\code{coalesce_cg.cp_coalesce_kind}. Deliberate simtgen programs closed
the first two (Fig.~\ref{fig:simtclosure}): structured divergence
towers hit depth 4 (4/4), and bank-hostile access patterns (all lanes
of a warp addressing one bank by construction) surfaced real
conflicts; both closures are folded into a suite bank (2026-09-09).
Three honesty notes are attached to
...
\begin{itemize}
	\item the bank-conflict bin's initial ``gap'' was in fact a
	      ...
	      simultaneously honest (169 conflict hits, 61 no-conflict, 70{,}460
	      idle); the closure credit is therefore shared between the probe fix
	      and the generator. A coverage definition must itself be validated
	      against the RTL's structure; the stimulus gap and the taxonomy defect
	      were disentangled by experiment.
	\item the coalescing classes, which an earlier isolated-merge report
	      attributed to simtgen, were re-examined and found already covered by
	      the existing coalescer probe and kernels before simtgen existed---an
	      attribution correction we report rather than quietly keep.
	\item the barrier and VOTE/SHFL generator axes closed nothing
	      because their targets were already fully covered by directed
	      kernels (Section~\ref{sec:stimulus}); their value is generator
	      completeness and seed-diversity robustness.
	      The marginal picture per generator kind: ninety additional
	      riscv-dv programs closed zero bins (the seed-farm result), while 57
	      simtgen programs (divergence and memory axes) closed the divergence
	      and bank-conflict targets---stimulus \emph{kind}, not volume, moves
	      this coverage model.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{D-extension elaboration, scoped by inspection.} The D
	      ...
	      confirmed by an EXT\_D-off recompile with a
	      coverpoint-by-coverpoint bin diff: zero scored bins change, the
	      entire delta confined to two weight-0 coverpoints already excluded
	      from every reported percentage. The
...
generator (the divergence and bank-conflict closures are folded into a
suite bank; the coalescing attribution and the coverage-model defect
found on the way are reported with their evidence).
