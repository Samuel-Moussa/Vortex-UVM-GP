...
**CURRENT MILESTONE: Phase A → A0 ✅ · A1(a) ✅ · A1(b) ✅ · A1(d) PINPOINTED ✅ · SimX fetch bug FIXED ✅ · OBS-002 CLOSED (sound load-compare, default-on) ✅ · OBS-009/010 ROOT-CAUSED (proven single-hart-test-in-multihart race, not a DUT bug) ✅. NEXT ACTION: DECIDE the real fix for multi-cluster verifiability of single-hart random tests (3 options below) — user wants a real fix, not error-suppression.**

### ▶▶ NEXT ACTION (exact, resumable — 2026-07-15) — DECISION POINT
**OBS-002 is DONE and committed (`97c4e30`).** Per-instruction LOAD-data compare is sound (region-filtered by exported SimX effective address) and **ON by default** (`+NO_LOCKSTEP_LOADS` escapes). Validated: `vecadd_lite` lockstep PASSED, 1035/1035 matched, **LOAD mismatch=0**, 74 in-region loads data-compared, 113 out-of-region filtered. Config-generic across NC/NCL/NW/NT (NT≤32 caveat inherited).

**OBS-009/010 are now PROVEN (decisive, `docs/investigations/SimX_2CL_no_fence_divergence.md` DECISIVE section):** the 2CL `no_fence`/`full_interrupt` divergences are **single-hart riscv-dv tests run on N shared-memory cores** → all cores hammer the same **fixed absolute** `.region_*`/stack (no `mhartid`, no fences) → genuine cross-core write-ordering races. Clincher: ELF init at the first diverging load address (`0x80020618`) = **0x7aea0e77 = the DUT value**; SimX reads a byte-zeroed 0x7a000e77 on cluster-1 only. DUT reads pristine init; SimX's fixed core-stepping order zeroes the byte between cluster-0 and cluster-1 reads. Both are valid executions of an architecturally-undefined program. **Not a DUT bug; not a fixable SimX staging bug.**

**These two tests now FAIL the lockstep at 2CL (18/16 UVM_ERROR) — EXPECTED** (they were already end-state-UNVERIFIABLE; load-compare adds the load-level root cause). Clean 1CL programs still pass 0 errors. **The user explicitly rejected suppression (`+NO_LOCKSTEP_LOADS`/test-exclusion) — wants a REAL fix. Pick ONE:**
1. **True lockstep cosim (RVVI-standard, industrial — aligns with Phase A1(c) rvvi_if):** drive SimX from the DUT's memory order — on a shared-region load, SimX consumes the DUT's loaded value instead of its own `g_ram`. Races resolve identically ⇒ verifiable. Cost: SimX becomes a step-driven follower for shared loads (can't independently verify a racy load's data — inherent).
2. **Config-scope single-hart random tests to 1C (methodologically correct, low effort):** run `no_fence`/`full_interrupt` at single-core (deterministic, they PASS); get multi-core coverage from multi-hart-AWARE directed kernels, not replicated single-hart binaries.
3. **Per-hart disjoint data regions:** regenerate riscv-dv with per-hart `.region_*`/stack offsets. More work; riscv-dv/linker-specific.

**Material that would help the decision (user offered):** RVVI spec (`github.com/riscv-verification/RVVI`) for the standard "ISS follows DUT load on shared memory" mechanism (informs option 1); riscv-dv docs on `riscv_no_fence_test`/multi-hart config (informs options 2/3); Vortex runtime/linker (per-hart region feasibility, option 3).

**Replay hexes (no regen):** pinned no_fence `results/20260710/run_125857_.../riscv_no_fence_test_0.hex`, full_interrupt `results/20260710/run_130835_.../riscv_full_interrupt_test_0.hex`; run `LOCKSTEP=1 make -C vortex_uvm_env sim TEST=random_instruction_stress_test PROGRAM=<abs hex> CLUSTERS=2 CORES=2 WARPS=4 THREADS=4 TIMEOUT=200000` (full `make sim` — 2CL image differs from 1CL; `LSU_DEBUG=1` dumps raw probe captures). **Rebuild note:** the enhanced-script DPI relink does NOT rebuild SimX `core.o` — after editing `simx_cosim_record.h`/`core.cpp`, run `make -C Vortex/sim/simx` first (header dep-tracked, but the run script skips the core rebuild).
