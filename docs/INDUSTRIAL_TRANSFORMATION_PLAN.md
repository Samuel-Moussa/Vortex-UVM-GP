...
> **STATUS 2026-07-14: the export described below is ALREADY IMPLEMENTED** (`554080e`). The live artifact is `simx_retire_t` ([simx_cosim_record.h](../Vortex/sim/simx/simx_cosim_record.h)) + `simx_cosim_pop()` DPI ([simx_dpi.cpp](../Vortex/sim/uvmsim/uvm_env/ref_model/simx_dpi.cpp):869) + `simx_retire_s`/import in `simx_pkg.sv`. The field design below matches it (add `mem_addr/mem_data` for A2). This appendix now documents the interface + the **SV consumer** to build (A0), not new C++.
