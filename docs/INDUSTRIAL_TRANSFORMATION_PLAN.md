...
**CURRENT MILESTONE: Phase A → A0 ✅ · A1(a) ✅ · A1(b) ✅ · A1(d) PINPOINTED ✅ · SimX fetch bug FIXED ✅ · OBS-002 CLOSED ✅ · OBS-009/010 ROOT-CAUSED ✅ · A1(e) RVVI LOAD-BUS ✅ (real fix — 2CL no_fence now VERIFIED, non-waiver, 0 UVM_ERROR). NEXT ACTION: end-state compare should source DUT values from the real `mem_model` (not `shadow_memory`) — focused follow-on with negative-test (fault-injection + dropped-store) re-validation.**

### ▶▶ NEXT ACTION (exact, resumable — 2026-07-16) — mem_model value-source switch
**A1(e) RVVI LOAD-BUS is DONE (ready to commit this session).** Real fix chosen (option 1) and implemented as a sound **two-pass trace-replay** (no step-follower rewrite needed — the whole DUT retire+load trace is captured before SimX runs). Result on pinned no_fence@2CL: pass-1 **20** racy loads → **138** cascade; pass-2 **residual 0** over **5432/5432**; deferred end-state compare (post-feed SimX) → racy word matches → **TEST PASSED, 0 UVM_ERROR**. Default (no `+LOCKSTEP_LOADFEED`) byte-identical (vecadd_lite 1035/1035 re-confirmed). Files: `Vortex/sim/simx/{cosim_loadfeed.h,emulator.cpp,execute.cpp}`, `ref_model/{simx_dpi.cpp,simx_pkg.sv}`, `lockstep_scoreboard.sv`, `vortex_scoreboard.sv` (deferred end-state), `scripts/simulate.sh`. Full writeup: `docs/investigations/SimX_2CL_no_fence_divergence.md` → "REAL FIX IMPLEMENTED".

**NEXT (user-directed 2026-07-16):** the forward end-state compare (`vortex_scoreboard.sv:684 compare_all_written`) sources DUT VALUES from `shadow_memory` (a reconstruction from snooped writes), not the real `mem_model.sv` (`dut_mem`, the actual DUT backing store). `mem_model` is ground truth; shadow predates the `dut_mem` handle (added with SB-DIR). **Switch the VALUE source to `dut_mem`, KEEP `shadow_valid` for the write-set/byte-mask** (needed to scope the compare) → end-state becomes real-DUT-mem vs real-post-feed-SimX. Guard: fall back to shadow if `dut_mem==null`. **Gate-0 sensitive** — the negative fault-injection (`+INJECT`) and dropped-store (`+DROP_STORE`) tests XOR/manipulate `dut_word`/`dut_mem`; MUST re-validate both stay RED on injection after the switch.
