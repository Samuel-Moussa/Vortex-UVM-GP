...
class vortex_virtual_sequence extends uvm_sequence;
    ...
    virtual task wait_for_execution_complete();
        ...
        if (cfg == null) begin
            `uvm_fatal("VIRT_SEQ", "wait_for_execution_complete: cfg is null - did pre_body() run?")
        end

        if (cfg.ebreak_event == null) begin
            `uvm_fatal("VIRT_SEQ", "cfg.ebreak_event is null - was vortex_config created correctly?")
        end

        timeout_ns = cfg.test_timeout_cycles * 10;

        `uvm_info("VIRT_SEQ", $sformatf("Waiting for EBREAK (timeout = %0d cycles / %0d ns)...", cfg.test_timeout_cycles, timeout_ns), UVM_MEDIUM)

        fork
            begin
                cfg.ebreak_event.wait_trigger();
                `uvm_info("VIRT_SEQ", "EBREAK event received - DUT execution complete", UVM_MEDIUM)
            end
            begin
                #(timeout_ns * 1ns);
                `uvm_error("VIRT_SEQ", $sformatf("Timeout: %0d cycles / %0d ns - DUT never signalled EBREAK", cfg.test_timeout_cycles, timeout_ns))
            end
        join_any
        disable fork;
    endtask
