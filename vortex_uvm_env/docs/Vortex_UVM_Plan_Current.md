...
| Date | SHA | Summary |
|------|-----|---------|
| 2026-06-30 | `0cfec34` | **Session 4 (Samuel): scoreboard byte-mask fix + host/DCR coverage + cp_id_route waiver + config-awareness audit.** **`4b7c55c`** scoreboard `shadow_valid` per-byte mask — sub-word stores into preinitialised `.data` no longer false-mismatch; recovered 3 riscv-dv tests (`non_compressed`/`rand_instr`/`ebreak`, 0 mismatches) while NEG injection still caught (Ahmad's lane, REVIEW). **`058ee9c`+`1b14388`** new `host_coverage_test` (real launch+wait then SimX-safe DCR sweep on idle DUT) → `dcr_config_cg.cp_data_magnitude`/`cp_addr` →100%, `cross_addr_data` 30→45%; added to run_suite. **`0cfec34`** evidence-based `ignore_bins` waiver for `cp_id_route`/`cross_type_route` residual {4,6,8,10,12,14}/{23,27,31}/{1,5,9,13} — PROVEN non-targetable (read route = `VX_allocator` lowest-free MSHR slot set by Ramulator release order; 29-run suite + 4 directed `axi_stress` runs reached depth slot 15 but never the evens) → both →100% reachable on re-merge; new `tests/kernel/axi_stress` kept as evidence (0 new route bins, not in suite). **Config-awareness audit:** collector config-aware for counts + AXI/MEM widths; route-tag ignores + `cp_active_warps` ceiling hardcoded to primary config (trip-wire), clusters ungated → all handed to Ahmad (`HANDOVER §A-C`). Full-suite re-merge launched to remeasure functional %. |
...
**1. `cp_id_route` (23%) + `cross_type_route` (16%) — ~157 bins, BIGGEST.** Route = `current_axi.id[ROUTE_W-1:0]`, ROUTE_W=6 (AXI_ID_W 50 − UUID 44). **Decoded empirically (DBG_ROUTE probe, vecadd_lite) + RTL (`VX_axi_adapter.sv`):**
  ...
  - **~~STIMULUS (reachable)~~ → DISPROVEN, now WAIVED (`0cfec34`, session 4):** even tbuf slots 4,6,8,10,12,14 + odd writes 23,27,31 are **NOT stimulus-targetable**. `route`(read)=MSHR free-list slot from `VX_allocator`'s lowest-free priority encoder; the emitted value is set purely by Ramulator acquire/release ordering, not by any kernel input. **Proof:** 29-run suite + 4 directed `axi_stress` runs (config-aware, same-bank 128B stride, ILP-maxed independent loads) reached read **depth slot 15** yet never emitted the evens; write counter hit 29 but skipped 23,27,31.
  - **(a) DONE (`3fdf359`):** config-aware `ignore_bins` (route≥32 + even≥16) on `cp_id_route`/`cross_type_route` → 64→24 / 128→32 bins (≈62.5%).
  - **(b) DONE (`0cfec34`):** evidence-based `ignore_bins route_emergent_read={4,6,8,10,12,14}` + `route_high_write_tag={23,27,31}` + cross `rd_emergent={1,5,9,13}`, with full PROOF block in `vortex_coverage_collector.sv`. → `cp_id_route` + `cross_type_route` reach **100% of reachable bins** on re-merge. **New `tests/kernel/axi_stress`** kept in-tree as reproducible evidence (NOT in run_suite — 0 new route bins). **Coverage-lane → flagged REVIEW: Ahmad** (`HANDOVER_Ahmad_coverage_pushup.md` §A).
...
**4. dcr_config_cg PARTLY DONE (`058ee9c`, session 4):** new `host_coverage_test` (real launch+wait, then SimX-safe DCR sweep on idle DUT) → `cp_data_magnitude` 50→**100%**, `cp_addr`→**100%**, `cross_addr_data` 30→45%. Added to run_suite (`1b14388`). **Left:** `cp_op_type` reset/load/read op-types (perturb the run — need clean pre-launch host flow), `cp_startup_align.unaligned` (faults SimX+DUT → waiver, REVIEW: Ahmad), `cross_addr_data` 45→100 (more SimX-safe ARGV_PTR1/MPM combos).
**5. status_performance stalls execute/decode/issue (50%)** — need RTL taps for the 3 remaining stall types. Status-agent + RTL (Ahmad/Steven).
**6. `cp_active_warps` 16% — sampling cadence, not stimulus.** `count_active_warps()` sampled at one point → never sees 1→2→3→4 ramp/drain. Needs periodic / mask-change sampling (coverage-collector lane → Ahmad, `HANDOVER §C`). I supply ramping/divergent stimulus once it captures transients.

**CONFIG-AWARENESS AUDIT (session 4) → Ahmad (`HANDOVER §B`):** collector is config-aware for counts (`cp_num_*` via `CFG_*`) + AXI/MEM widths (`$bits(id)`, `ROUTE_W`, `AXI_NATIVE_SIZE`), but route-tag ignore *values* + `cp_active_warps` ceiling are hardcoded to the primary config (trip-wire flagged), and **clusters is neither covered nor gated** (`CFG_CLUSTERS` declared-but-unused, no `cp_num_clusters`). Fixes: derive route ignores from `TAG_BUFFER_SIZE`/`ROUTE_W`; span `cp_active_warps` over `` `NUM_WARPS ``; add `cp_num_clusters`.
