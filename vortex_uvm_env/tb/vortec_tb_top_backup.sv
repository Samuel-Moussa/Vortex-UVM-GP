...
//     // // ✅ CRITICAL FIX: Force startup_addr at time 0 and HOLD IT THROUGH RESET
//     // force dut.g_clusters[0].cluster.g_sockets[0].socket.dcr_data.dcrs.startup_addr = 64'h80000000;
    
//     #1ns;
//     $display("[TB_TOP @ %0t] ✅ Forced startup_addr = 0x80000000", $time);
    
//     // ⚠️ CRITICAL: Wait for RESET to COMPLETE before releasing!
//     wait(reset_n == 1'b0);  // Wait for reset to assert
//     wait(reset_n == 1'b1);  // Wait for reset to release
//     repeat(10) @(posedge clk);  // Extra safety margin
    
//     release dut.g_clusters[0].cluster.g_sockets[0].socket.dcr_data.dcrs.startup_addr;
    
//     $display("[TB_TOP @ %0t] Released startup_addr force AFTER reset complete", $time);
// end
    //==========================================================================
...
forever begin
    ...
    if (vif.mem_if.mem_responder_cb.req_valid[0]) begin
        ...
        if (vif.mem_if.mem_responder_cb.req_rw[0]) begin
            // Write request
            automatic bit [31:0] addr   = vif.mem_if.mem_responder_cb.req_addr[0];
            ...
            $display("[TB_TOP @ %0t] MEM READ:  addr=0x%08h tag=0x%01h", 
                     $time,
                     vif.mem_if.mem_responder_cb.req_addr[0],
                     vif.mem_if.mem_responder_cb.req_tag[0]);
        end
        ...
        vif.mem_if.mem_responder_cb.rsp_data[0]  <= memory.read_word(vif.mem_if.mem_responder_cb.req_addr[0]);
        vif.mem_if.mem_responder_cb.rsp_tag[0]   <= vif.mem_if.mem_responder_cb.req_tag[0];
        
        $display("[TB_TOP @ %0t] MEM RESP:  data=0x%08h tag=0x%01h", 
                 $time,
                 memory.read_word(vif.mem_if.mem_responder_cb.req_addr[0]),
                 vif.mem_if.mem_responder_cb.req_tag[0]);
    end else begin
