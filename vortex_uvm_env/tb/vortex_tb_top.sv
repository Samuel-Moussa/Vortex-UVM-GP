...
//   FIX-5: mem_model registered in config_db BEFORE run_test() is called.
//          Previously mem_model was created in a separate initial block that
//          ran in parallel with the UVM setup block. Questa's scheduler
//          allowed run_test() → build_phase → connect_phase to execute before
//          the mem_model initial block ran its config_db::set calls.
//          Result: AXI monitor found no mem_model → R-beat check disabled.
//          Fix: merged mem_model creation into the single UVM config initial
//          block, registered FIRST, then virtual interfaces, then run_test().
//
...
//   FIX-4: Removed assign vif.axi_if.* for DUT-driven signals with inline
//          initial values. arready driven only from always_ff.
//
////////////////////////////////////////////////////////////////////////////////
...
module vortex_tb_top;
    ...
    //==========================================================================
    // MEMORY MODEL — declared here so all initial blocks below can reference it
    //==========================================================================
    ...
    `ifdef USE_AXI_WRAPPER
        ...
        // AXI Read Channel — FIX-1: combinational arready
    ...
    //==========================================================================
    // UVM CONFIGURATION DATABASE SETUP — FIX-5
    //
    // mem_model is created and registered FIRST in this block, before any
    // virtual interface or run_test() call. This guarantees that when UVM's
    // build_phase → connect_phase runs (triggered by run_test()), the
    // mem_model is already present in config_db and the AXI monitor can
    // retrieve it. Previously mem_model lived in a separate initial block
    // that raced against run_test() and always lost in Questa.
    //==========================================================================

    // initial begin
    //     // ── STEP 1: create and register mem_model ──────────────────────────
    ...
    //     uvm_config_db# (mem_model)::set(uvm_root::get(), "*",  "mem_model", memory);
    //      $display("[TB_TOP @ %0t] ✓ mem_model created and registered in config_db", $time);

    //     // ── STEP 2: register virtual interfaces ────────────────────────────
    ...
    //     // ── STEP 3: launch UVM — build_phase will find mem_model ───────────
    //     run_test();
    // end
    initial begin
        // ── STEP 1: create and register mem_model ──────────────────────────
        // Single wildcard set — visible to every UVM component via get(null,"*")
        memory = mem_model::type_id::create("memory");
        uvm_config_db#(mem_model)::set(uvm_root::get(), "*", "mem_model", memory);
        $display("[TB_TOP @ %0t] mem_model created and registered in config_db", $time);
